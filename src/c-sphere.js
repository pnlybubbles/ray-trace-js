...
class CSphere extends CObject {
  ...
  isAcross(ray) {
    ...
    const d = Math.pow(vac, 2) - vec3.sqrLen(ray.v) * (aclen - Math.pow(r, 2));
    if (d >= 0) {
      const hitPos = vec3.add(vec3.create(), ray.o, vec3.scale(vec3.create(), ray.v, (-vac - Math.sqrt(d)) / aclen));
      const normal = vec3.normalize(vec3.create(), vec3.sub(vec3.create(), hitPos, this.pos));
      return new Intersection(hitPos, normal, vec3.fromValue(1, 1, 1));
    }
    return null;
  }
}
